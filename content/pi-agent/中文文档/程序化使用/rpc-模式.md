---
title: "RPC 模式"
description: "Pi-Coding-Agent 中文文档 — RPC 模式"
weight: 200
created: 2025-07-16
---
...
**针对 Node.js/TypeScript 用户的说明**: 如果你正在构建 Node.js 应用程序，建议直接使用 `@earendil-works/pi-coding-agent` 中的 `AgentSession`，而不是启动子进程。API 请参见 [`src/core/agent-session.ts`](../src/core/agent-session.ts)。基于子进程的 TypeScript 客户端请参见 [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts)。
...
- **响应 (Responses)**: `type: "response"` 的 JSON 对象，指示命令成功/失败
...
- `"steer"`: 在代理运行时将消息排队。在当前助手轮次执行完其工具调用后、下一次 LLM 调用之前投递。
...
`success: true` 表示提示已被接受、排队或立即处理。`success: false` 表示提示在接受前被拒绝。接受后的失败通过正常的事件和消息流报告，而不是作为相同请求 ID 的第二个 `response`。
...
关于如何控制引导消息的处理，请参见 [set_steering_mode](#set_steering_mode)。
...
关于如何控制后续消息的处理，请参见 [set_follow_up_mode](#set_follow_up_mode)。
...
- `"all"`: 在当前助手轮次执行完其工具调用后，投递所有引导消息
- `"one-at-a-time"`: 每个完成的助手轮次投递一条引导消息（默认）
...
- `"all"`: 代理完成时投递所有后续消息
- `"one-at-a-time"`: 每次代理完成时投递一条后续消息（默认）
...
事件在代理运行期间以 JSON 行形式流式输出到 stdout。事件**不**包含 `id` 字段（只有响应包含）。
...
- **对话框方法** (`select`、`confirm`、`input`、`editor`)：在 stdout 上发出 `extension_ui_request`，并阻塞等待客户端通过 stdin 发送带有匹配 `id` 的 `extension_ui_response`。
- **即发即弃方法** (`notify`、`setStatus`、`setWidget`、`setTitle`、`set_editor_text`)：在 stdout 上发出 `extension_ui_request`，但不期望回复。客户端可以显示该信息或忽略它。
...
- `custom()` 返回 `undefined`
- `setWorkingMessage()`、`setWorkingIndicator()`、`setFooter()`、`setHeader()`、`setEditorComponent()`、`setToolsExpanded()` 为无操作
- `getEditorText()` 返回 `""`
...
完整的交互式示例请参见 [`test/rpc-example.ts`](../test/rpc-example.ts)，或参见 [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts) 获取类型化客户端实现。
